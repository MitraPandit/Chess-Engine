...
GameBoard.hisPly = 0; // history ply
GameBoard.history = [];  // for makemove function, stores history of prev moves
GameBoard.ply = 0;
GameBoard.enPas = 0;
GameBoard.castlePerm = 0;
GameBoard.material = new Array(2); // val of material of each side
GameBoard.pceNum = new Array(13); // how many of pce type we have,,,, indexed for pce
GameBoard.pList = new Array(14 * 10);
GameBoard.posKey = 0; // represents unique position on the board, check for repetition
...
/*
Starts from 8th Rank - Big Endian Notation
Rank8/Rank7/...../Rank1 side(w/b) (castleperm) (enpas sq) 50move ply
*/

/*
loop(pieces[])
if(piece on sq == side to move)
then genmoves() for piece on sq  but this is not efficient
 
so maintain piecelist
 
for a given piece we want to know what square the piece is on
...
function UpdateListsMaterial() {
    ...
    for (index = 0; index < 14 * 120; ++index) {
        GameBoard.pList[index] = PIECES.EMPTY;   // set plist as empty
    }

    for (index = 0; index < 2; ++index) {
        GameBoard.material[index] = 0; // material as zero
    }

    for (index = 0; index < 13; ++index) {
        GameBoard.pceNum[index] = 0; // piece number zero as well
    }
    ...
    for (index = 0; index < 64; ++index) {
        ...
        if (piece != PIECES.EMPTY) {
            ...
            GameBoard.pceNum[piece]++; // update no of pieces
        }
    }
...
function ResetBoard() {
    ...
    for (index = 0; index < BRD_SQ_NUM; ++index) {
        GameBoard.pieces[index] = SQUARES.OFFBOARD; // set all pieces on 120 squares as offboard
    }

    for (index = 0; index < 64; ++index) {
        GameBoard.pieces[SQ120(index)] = PIECES.EMPTY; // internal 64 sq as empty
    }
