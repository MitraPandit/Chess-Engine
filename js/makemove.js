...
function MakeMove(move) {
    ...
    if ((move & MFLAGEP) != 0) { // dealing with enpas move
        if (side == COLOURS.WHITE) {
            ClearPiece(to - 10);
        } else {
            ClearPiece(to + 10);
        }
    } else if ((move & MFLAGCA) != 0) { // dealing with castle move and moving rook to appropriate square
        switch (to) {
            case SQUARES.C1:
    ...
    GameBoard.castlePerm &= CastlePerm[from]; // hash in the new castle perm
    GameBoard.castlePerm &= CastlePerm[to];
    GameBoard.enPas = SQUARES.NO_SQ;

    HASH_CA(); // hash out the current castling permission
    ...
    GameBoard.hisPly++; // store every info in history 
    GameBoard.ply++;
